 ...
 */
const FeatureExtractor_1 = __webpack_require__("23ZG");
const FrequencyDomainAdapter_1 = __webpack_require__("2QOl");
class FeatureExtractorSynchronousService {
    constructor(fftFactory, ...factories) {
        FeatureExtractorSynchronousService.sanitiseStaticData(factories);
        this.factories = new Map(factories.map(plugin => [plugin.metadata.key, plugin]));
        this.loaded = new Map();
    ...
    load(request) {
        ...
        const isInputDomainAdapted = request.adapterFlags.length > 0
            && (request.adapterFlags.includes(FeatureExtractor_1.AdapterFlags.AdaptAll)
                || request.adapterFlags.includes(FeatureExtractor_1.AdapterFlags.AdaptAllSafe)
                || request.adapterFlags.includes(FeatureExtractor_1.AdapterFlags.AdaptInputDomain));
        const factory = this.factories.get(request.key);
        const metadata = factory.metadata;
        const extractor = metadata.inputDomain === FeatureExtractor_1.InputDomain.FrequencyDomain && isInputDomainAdapted
            ? new FrequencyDomainAdapter_1.FrequencyDomainAdapter(factory.create(request.inputSampleRate), this.fftFactory, request.inputSampleRate, FrequencyDomainAdapter_1.ProcessInputAdjustmentMethod.Timestamp)
    ...
    configure(request) {
        ...
        const response = plugin.extractor.configure(request.configuration);
    ...
    process(request) {
        ...
        const numberOfInputs = request.processInput.inputBuffers.length;
        const metadata = plugin.metadata;
        if (numberOfInputs < metadata.minChannelCount || numberOfInputs > metadata.maxChannelCount)
    ...
    static sanitiseStaticData(factories) {
        ...
        factories.forEach(plugin => {
            if (typeof plugin.metadata.inputDomain === "string") {
                plugin.metadata.inputDomain = FeatureExtractor_1.InputDomain[plugin.metadata.inputDomain];
            }
        });
    }
}
exports.FeatureExtractorSynchronousService = FeatureExtractorSynchronousService;
class FakeAsyncService {
    constructor(service) {
        this.service = service;
    }
    list(request) {
        return this.request(request, (req) => this.service.list(req));
    }
    load(request) {
        return this.request(request, (req) => this.service.load(req));
    }
    configure(request) {
        return this.request(request, (req) => this.service.configure(req));
    }
    process(request) {
        return this.request(request, (req) => this.service.process(req));
    }
    finish(request) {
        return this.request(request, (req) => this.service.finish(req));
    }
...
exports.FakeAsyncService = FakeAsyncService;
class FeatureExtractorService extends FakeAsyncService {
    constructor(fftFactory, ...factories) {
        super(new FeatureExtractorSynchronousService(fftFactory, ...factories));
    }
}
exports.FeatureExtractorService = FeatureExtractorService;
//# sourceMappingURL=FeatureExtractorService.js.map
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_piper_servers_WebWorkerStreamingServer__ = __webpack_require__("oPBu");
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_piper_servers_WebWorkerStreamingServer___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_1_piper_servers_WebWorkerStreamingServer__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2_piper_StreamingService__ = __webpack_require__("e4w8");
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2_piper_StreamingService___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_2_piper_StreamingService__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3_rxjs_Observable__ = __webpack_require__("rCTf");
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3_rxjs_Observable___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_3_rxjs_Observable__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4__FeatureReducers__ = __webpack_require__("lvDJ");
/**
...
class AggregateStreamingService {
    ...
    dispatch(method, request) {
        ...
        return this.services.has(key) ?
            this.services.get(key)[method](request) : __WEBPACK_IMPORTED_MODULE_3_rxjs_Observable__["Observable"].throw('Invalid key');
    }
}
class ThrottledReducingAggregateService extends AggregateStreamingService {
    ...
    dispatch(method, request) {
        ...
        return super.dispatch(method, request)
            ...
            return __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_4__FeatureReducers__["a" /* streamingResponseReducer */])(acc, value);
        })
            .filter(val => {
...
class FeatureExtractionWorker {
    constructor(workerScope, requireJs) {
        ...
        this.server = new __WEBPACK_IMPORTED_MODULE_1_piper_servers_WebWorkerStreamingServer__["WebWorkerStreamingServer"](this.workerScope, this.service);
    }
                ...
                case 'import':
                    ...
                    if (this.remoteLibraries.has(key)) {
                        this.requireJs([this.remoteLibraries.get(key)], (plugin) => {
                            ...
                            const service = new __WEBPACK_IMPORTED_MODULE_2_piper_StreamingService__["PiperStreamingService"](isEmscriptenModule ? new __WEBPACK_IMPORTED_MODULE_0_piper__["PiperVampService"](lib) : lib // TODO
                            );
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__app_services_feature_extraction_FeatureExtractionWorker__ = __webpack_require__("/3Ld");
/**
 * Created by lucas on 01/12/2016.
 */
...
const modules = {
    'feature-extraction-worker': __WEBPACK_IMPORTED_MODULE_0__app_services_feature_extraction_FeatureExtractionWorker__["a" /* default */]
};
if (typeof self.importScripts === 'function' /* in a worker */) {
...
module.exports = __webpack_require__("/WRh");
...
(function (InputDomain) {
    InputDomain[InputDomain["TimeDomain"] = 0] = "TimeDomain";
    InputDomain[InputDomain["FrequencyDomain"] = 1] = "FrequencyDomain";
})(InputDomain = exports.InputDomain || (exports.InputDomain = {}));
var SampleType;
(function (SampleType) {
    SampleType[SampleType["OneSamplePerStep"] = 0] = "OneSamplePerStep";
    SampleType[SampleType["FixedSampleRate"] = 1] = "FixedSampleRate";
    SampleType[SampleType["VariableSampleRate"] = 2] = "VariableSampleRate";
})(SampleType = exports.SampleType || (exports.SampleType = {}));
var AdapterFlags;
(function (AdapterFlags) {
    AdapterFlags[AdapterFlags["AdaptNone"] = 0] = "AdaptNone";
    AdapterFlags[AdapterFlags["AdaptInputDomain"] = 1] = "AdaptInputDomain";
    AdapterFlags[AdapterFlags["AdaptChannelCount"] = 2] = "AdaptChannelCount";
    AdapterFlags[AdapterFlags["AdaptBufferSize"] = 3] = "AdaptBufferSize";
    AdapterFlags[AdapterFlags["AdaptAllSafe"] = 4] = "AdaptAllSafe";
    AdapterFlags[AdapterFlags["AdaptAll"] = 5] = "AdaptAll";
})(AdapterFlags = exports.AdapterFlags || (exports.AdapterFlags = {}));
...
Object.defineProperty(exports, "__esModule", { value: true });
const FftUtilities_1 = __webpack_require__("vgcF");
const ProcessInputAdjuster_1 = __webpack_require__("xmxW");
var ProcessInputAdjustmentMethod;
(function (ProcessInputAdjustmentMethod) {
    ProcessInputAdjustmentMethod[ProcessInputAdjustmentMethod["Timestamp"] = 0] = "Timestamp";
    ProcessInputAdjustmentMethod[ProcessInputAdjustmentMethod["Buffer"] = 1] = "Buffer";
})(ProcessInputAdjustmentMethod = exports.ProcessInputAdjustmentMethod || (exports.ProcessInputAdjustmentMethod = {}));
class FrequencyDomainAdapter {
    constructor(extractor, fftFactory, sampleRate, adjustmentMethod) {
    ...
    configure(configuration) {
        ...
        this.adjuster = this.adjustmentMethod === ProcessInputAdjustmentMethod.Buffer
            ? new ProcessInputAdjuster_1.ProcessInputBuffersAdjuster(configuration)
            : new ProcessInputAdjuster_1.ProcessInputTimestampAdjuster(configuration.framing.blockSize, this.sampleRate);
    ...
    process(block) {
        const forwardFft = (channel) => this.fft.forward(FftUtilities_1.cyclicShiftInPlace(FftUtilities_1.applyHannWindowTo(channel)));
        const timeAdjustedBlock = this.adjuster.adjust(block);
...
exports.FrequencyDomainAdapter = FrequencyDomainAdapter;
//# sourceMappingURL=FrequencyDomainAdapter.js.map
 ...
 */
const base64 = __webpack_require__("EKta");
const FeatureExtractor_1 = __webpack_require__("23ZG");
var Serialise;
(function (Serialise) {
    function ListRequest(request, tag) {
        return toTransport({ method: "list", params: request }, tag);
    }
    Serialise.ListRequest = ListRequest;
    function ListResponse(response, tag) {
        return toTransport({ method: "list", result: toWireListResponse(response) }, tag);
    }
    Serialise.ListResponse = ListResponse;
    function LoadRequest(request, tag) {
        return toTransport({ method: "load", params: toWireLoadRequest(request) }, tag);
    }
    Serialise.LoadRequest = LoadRequest;
    function LoadResponse(response, tag) {
        return toTransport({ method: "load", result: toWireLoadResponse(response) }, tag);
    }
    Serialise.LoadResponse = LoadResponse;
    function ConfigurationRequest(request, tag) {
        return toTransport({ method: "configure", params: toWireConfigurationRequest(request) }, tag);
    }
    Serialise.ConfigurationRequest = ConfigurationRequest;
    function ConfigurationResponse(response, tag) {
        return toTransport({ method: "configure", result: toWireConfigurationResponse(response) }, tag);
    }
    Serialise.ConfigurationResponse = ConfigurationResponse;
    function ProcessRequest(request, asBase64 = true, tag) {
        return toTransport({ method: "process", params: toWireProcessRequest(request, asBase64) }, tag);
    }
    Serialise.ProcessRequest = ProcessRequest;
    function ProcessResponse(response, asBase64 = true, tag) {
        return toTransport({ method: "process", result: toWireProcessResponse(response, asBase64) }, tag);
    }
    Serialise.ProcessResponse = ProcessResponse;
    function FinishRequest(request, tag) {
        return toTransport({ method: "finish", params: request }, tag);
    }
    Serialise.FinishRequest = FinishRequest;
    function FinishResponse(response, asBase64 = true, tag) {
        return toTransport({ method: "finish", result: toWireProcessResponse(response, asBase64) }, tag);
    }
    Serialise.FinishResponse = FinishResponse;
})(Serialise = exports.Serialise || (exports.Serialise = {}));
var Deserialise;
(function (Deserialise) {
    function ListRequest(request) {
        return toListRequest(fromTransport(request));
    }
    Deserialise.ListRequest = ListRequest;
    function ListResponse(response) {
        return toListResponse(fromTransport(response));
    }
    Deserialise.ListResponse = ListResponse;
    function LoadRequest(request) {
        return toLoadRequest(fromTransport(request));
    }
    Deserialise.LoadRequest = LoadRequest;
    function LoadResponse(response) {
        return toLoadResponse(fromTransport(response));
    }
    Deserialise.LoadResponse = LoadResponse;
    function ConfigurationRequest(request) {
        return toConfigurationRequest(fromTransport(request));
    }
    Deserialise.ConfigurationRequest = ConfigurationRequest;
    function ConfigurationResponse(response) {
        return toConfigurationResponse(fromTransport(response));
    }
    Deserialise.ConfigurationResponse = ConfigurationResponse;
    function ProcessRequest(request) {
        return toProcessRequest(fromTransport(request));
    }
    Deserialise.ProcessRequest = ProcessRequest;
    function ProcessResponse(response) {
        return toProcessResponse(fromTransport(response));
    }
    Deserialise.ProcessResponse = ProcessResponse;
    function FinishRequest(request) {
        return fromTransport(request);
    }
    Deserialise.FinishRequest = FinishRequest;
    function FinishResponse(response) {
        return ProcessResponse(response);
    }
    Deserialise.FinishResponse = FinishResponse;
})(Deserialise = exports.Deserialise || (exports.Deserialise = {}));
function toTransport(obj, tag) {
    const value = tag != null ? Object.assign({}, obj, { id: tag }) : obj;
    return JSON.stringify(value);
}
function fromTransport(buffer) {
    const response = typeof buffer === 'string' ?
        JSON.parse(buffer) : buffer;
    if (response.error)
        throw new Error(response.error.message);
    return response.result || response.params;
}
function toWireListResponse(response) {
    return {
        available: response.available.map(data => Object.assign({}, data, {
            inputDomain: FeatureExtractor_1.InputDomain[data.inputDomain]
        }))
    };
}
function toListRequest(request) {
    return request;
}
function toListResponse(response) {
    return {
        available: response.available.map(data => Object.assign({}, data, {
            inputDomain: parseInt(FeatureExtractor_1.InputDomain[data.inputDomain])
        }))
    };
}
function toWireLoadRequest(request) {
    return Object.assign({}, request, { adapterFlags: request.adapterFlags.map(flag => FeatureExtractor_1.AdapterFlags[flag]) });
}
function toLoadRequest(request) {
    return {
        key: request.key,
        inputSampleRate: request.inputSampleRate,
        adapterFlags: request.adapterFlags.map(flag => parseInt(FeatureExtractor_1.AdapterFlags[flag]))
    };
}
function toWireLoadResponse(response) {
    const staticData = response.staticData;
    return {
        handle: response.handle,
        staticData: Object.assign({}, staticData, { inputDomain: FeatureExtractor_1.InputDomain[staticData.inputDomain] }),
        defaultConfiguration: toWireConfiguration(response.defaultConfiguration)
    };
}
function toLoadResponse(response) {
    const staticData = response.staticData;
    return {
        handle: response.handle,
        staticData: Object.assign({}, staticData, { inputDomain: parseInt(FeatureExtractor_1.InputDomain[staticData.inputDomain]) }),
        defaultConfiguration: toConfiguration(response.defaultConfiguration)
    };
}
function toWireConfigurationRequest(request) {
    return {
        handle: request.handle,
        configuration: toWireConfiguration(request.configuration)
    };
}
function toConfigurationRequest(request) {
    return {
        handle: request.handle,
        configuration: toConfiguration(request.configuration)
    };
}
function toWireConfigurationResponse(response) {
    return Object.assign({}, response, {
        outputList: response.outputList.map(output => Object.assign({}, output, {
            configured: Object.assign({}, output.configured, {
                sampleType: FeatureExtractor_1.SampleType[output.configured.sampleType]
            })
        }))
    });
}
function toConfigurationResponse(response) {
    return Object.assign({}, response, {
        outputList: response.outputList.map(output => Object.assign({}, output, {
            configured: Object.assign({}, output.configured, {
                sampleType: parseInt(FeatureExtractor_1.SampleType[output.configured.sampleType])
            })
        }))
    });
}
function toWireConfiguration(config) {
...
function toConfiguration(config) {
    return config.parameterValues == null
        ? { channelCount: config.channelCount, framing: config.framing }
        : Object.assign({}, config, {
...
function toWireProcessRequest(request, asBase64) {
    const nChannels = request.processInput.inputBuffers.length;
    const inputBuffers = new Array(nChannels);
    if (asBase64) {
...
function toProcessRequest(request) {
    const hasBase64InputBuffer = typeof request.processInput.inputBuffers[0] === "string";
    const nChannels = request.processInput.inputBuffers.length;
    let inputBuffers = new Array(nChannels);
    const wireBuffers = request.processInput.inputBuffers;
    if (hasBase64InputBuffer) {
        for (let i = 0; i < nChannels; ++i)
            inputBuffers[i] = fromBase64(wireBuffers[i]);
    }
    else {
        for (let i = 0; i < nChannels; ++i)
            inputBuffers[i] = new Float32Array(wireBuffers[i]);
    }
    return {
        handle: request.handle,
        processInput: {
...
function toWireProcessResponse(response, asBase64) {
    let wireFeatureSet = {};
    const keys = Array.from(response.features.keys());
    for (let i = 0, nKeys = keys.length; i < nKeys; ++i) {
        const featureList = response.features.get(keys[i]);
        const nFeatures = featureList.length;
...
function toProcessResponse(response) {
    const features = new Map();
    const wireFeatures = response.features;
    const keys = Object.keys(wireFeatures);
    for (let i = 0, nKeys = keys.length; i < nKeys; ++i)
...
function convertWireFeatureList(wfeatures) {
    const nFeatures = wfeatures.length;
...
function toBase64(values) {
    ...
    return base64.fromByteArray(new Uint8Array(values.buffer, values.byteOffset, values.byteLength));
}
...
const FeatureExtractor_1 = __webpack_require__("23ZG");
class VariableSampleRateFeatureTimeAdjuster {
    ...
    adjust(feature) {
        ...
        const hasSampleRate = this.descriptor.hasOwnProperty("sampleRate") && this.descriptor.sampleRate !== 0.0;
        if (!feature.hasOwnProperty("duration"))
            feature.duration = hasSampleRate ? Timestamp_1.fromFrames(1, this.descriptor.sampleRate) : { s: 0, n: 0 };
    }
}
exports.VariableSampleRateFeatureTimeAdjuster = VariableSampleRateFeatureTimeAdjuster;
class FixedSampleRateFeatureTimeAdjuster {
    constructor(descriptor) {
        this.descriptor = descriptor;
        if (!descriptor.hasOwnProperty("sampleRate") || descriptor.sampleRate === 0.0)
            throw new Error("OutputDescriptor must provide a sample rate.");
        this.lastFeatureIndex = -1;
    }
    adjust(feature) {
        const sr = this.descriptor.sampleRate;
        const featureIndex = feature.hasOwnProperty("timestamp") ? Math.round(Timestamp_1.toSeconds(feature.timestamp) * sr) : this.lastFeatureIndex + 1;
        feature.timestamp = Timestamp_1.fromFrames(featureIndex, sr);
        feature.duration = feature.hasOwnProperty("duration") ? Timestamp_1.fromFrames(Math.round(Timestamp_1.toSeconds(feature.duration) * sr), sr) : { s: 0, n: 0 };
        this.lastFeatureIndex = featureIndex;
    }
}
exports.FixedSampleRateFeatureTimeAdjuster = FixedSampleRateFeatureTimeAdjuster;
class OneSamplePerStepFeatureTimeAdjuster {
    constructor(stepSizeSeconds) {
        if (stepSizeSeconds === undefined)
            throw new Error("Host must provide the step size (seconds).");
        this.stepSizeSeconds = stepSizeSeconds;
        this.lastFeatureIndex = -1;
    }
    adjust(feature, inputTimestamp) {
        ...
        if (isValidTimestamp)
            this.lastFeatureIndex = Math.round(Timestamp_1.toSeconds(feature.timestamp) / this.stepSizeSeconds);
        delete feature.duration; // host should ignore duration
    }
    calculateNextTimestamp() {
        return Timestamp_1.fromSeconds(++this.lastFeatureIndex * this.stepSizeSeconds);
    }
}
exports.OneSamplePerStepFeatureTimeAdjuster = OneSamplePerStepFeatureTimeAdjuster;
function createFeatureTimeAdjuster(descriptor, stepSizeSeconds) {
    switch (descriptor.sampleType) {
        case FeatureExtractor_1.SampleType.OneSamplePerStep:
            return new OneSamplePerStepFeatureTimeAdjuster(stepSizeSeconds);
        case FeatureExtractor_1.SampleType.VariableSampleRate:
            return new VariableSampleRateFeatureTimeAdjuster(descriptor);
        case FeatureExtractor_1.SampleType.FixedSampleRate:
...
exports.createFeatureTimeAdjuster = createFeatureTimeAdjuster;
//# sourceMappingURL=FeatureTimeAdjuster.js.map
...
const PiperClient_1 = __webpack_require__("hdBY");
function* segment(blockSize, stepSize, audioData) {
    let nStep = 0;
    const nSteps = audioData[0].length / stepSize;
    while (nStep < nSteps) {
        const start = nStep++ * stepSize;
        const stop = start + blockSize;
...
function* toProcessInputStream(stream, stepSize) {
    let nFrame = 0;
    for (let frame of stream.frames) {
        yield {
            timestamp: Timestamp_1.fromFrames(nFrame, stream.format.sampleRate),
            inputBuffers: frame
        };
        nFrame += stepSize;
    }
}
exports.toProcessInputStream = toProcessInputStream;
function determineConfiguration(defaultConfig, overrides) {
    let blockSize = overrides.blockSize || defaultConfig.framing.blockSize || 1024;
    let stepSize = overrides.stepSize || defaultConfig.framing.stepSize || blockSize;
    let channelCount = overrides.channelCount || defaultConfig.channelCount || 1; // TODO is 1 okay?
    let config = {
...
function loadAndConfigureExtractor(extractor, defaultConfig, channelCount, params = new Map(), args = {}) {
...
function* processConfiguredExtractor(frames, sampleRate, stepSize, extractor, outputs) {
    let nFrame = 0;
    const lazyOutput = function* (features) {
    ...
    for (let frame of frames) {
        const features = extractor.process({
            timestamp: Timestamp_1.fromFrames(nFrame, sampleRate),
            inputBuffers: frame
        });
...
exports.processConfiguredExtractor = processConfiguredExtractor;
function deduceShape(descriptor) {
    const isList = descriptor.hasDuration
        || descriptor.sampleType === FeatureExtractor_1.SampleType.VariableSampleRate
        || !descriptor.binCount;
...
function getFeatureStepDuration(inputSampleRate, stepSize, descriptor) {
    switch (descriptor.sampleType) {
        case FeatureExtractor_1.SampleType.OneSamplePerStep:
...
function collect(createAudioStreamCallback, streamFormat, createFeatureExtractorCallback, extractorKey, outputId, params, args = {}) {
    ...
    const extractor = createFeatureExtractorCallback(streamFormat.sampleRate, extractorKey);
    const [config, outputs] = loadAndConfigureExtractor(extractor, extractor.getDefaultConfiguration(), streamFormat.channelCount, params, args);
    const stream = createAudioStreamCallback(config.framing.blockSize, config.framing.stepSize, streamFormat);
    outputId = outputId ? outputId : outputs.outputs.keys().next().value;
    if (!outputs.outputs.has(outputId))
...
function* process(createAudioStreamCallback, streamFormat, createFeatureExtractorCallback, extractorKey, outputId, params, args = {}) {
    ...
    const descriptor = outputs.outputs.get(outputId);
    const lazyOutputs = processConfiguredExtractor(stream.frames, stream.format.sampleRate, config.framing.stepSize, extractor, [outputId]);
    const adjuster = FeatureTimeAdjuster_1.createFeatureTimeAdjuster(descriptor, getFeatureStepDuration(stream.format.sampleRate, config.framing.stepSize, descriptor));
...
function batchProcess(blocks, process, finish) {
    const processThunks = [...blocks].map(block => () => process(block))
        .concat([finish]);
...
function concatFeatures(running, nextBlock) {
...
function createOrConcat(data, key, map) {
    if (map.has(key)) {
        const a = map.get(key);
        a.push.apply(a, data);
    }
...
function loadAndConfigure(request, service) {
    const load = (request) => (response) => {
        ...
        return service.load({
            ...
            adapterFlags: [FeatureExtractor_1.AdapterFlags.AdaptAllSafe]
        });
    };
    const configure = (request) => (res) => {
        const config = determineConfiguration(res.defaultConfiguration, {
...
exports.loadAndConfigure = loadAndConfigure;
class PiperSimpleClient {
    constructor(service) {
        this.client = new PiperClient_1.PiperClient(service);
    }
    list(request) {
    ...
    processAndFinish(request, forceList = true) {
        ...
        return (res) => {
            ...
            const blocks = toProcessInputStream({
                frames: segment(res.configuredBlockSize, res.configuredStepSize, request.audioData),
                format: request.audioFormat
            }, res.configuredStepSize);
            return batchProcess(blocks, (block) => this.client.process({
                handle: res.handle,
                processInput: block
            }).then(response => response.features), () => this.client.finish({ handle: res.handle }).then(res => res.features)).then(featureSet => {
                const features = featureSet.get(res.configuredOutputId) || [];
                return forceList ? {
                    features: {
                        shape: "list",
                        collected: features
...
exports.PiperSimpleClient = PiperSimpleClient;
//# sourceMappingURL=HigherLevelUtilities.js.map
...
Object.defineProperty(exports, "__esModule", { value: true });
const HigherLevelUtilities_1 = __webpack_require__("UNpC");
...
function segmentAndExtractAsync(request, service, config, onFeaturesExtracted, onComplete) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const processInputs = HigherLevelUtilities_1.toProcessInputStream({
            frames: HigherLevelUtilities_1.segment(config.configuredBlockSize, config.configuredStepSize, request.audioData),
            format: request.audioFormat
        ...
        const response = yield service.finish({
            handle: config.handle
        });
        onFeaturesExtracted(response.features);
        onComplete();
...
function streamFeatures(request, service, config) {
    ...
    createResponseObservable(request) {
        return rxjs_1.Observable.fromPromise(HigherLevelUtilities_1.loadAndConfigure(request, this.client)).flatMap((config) => {
            return streamFeatures(request, this.client, config)
                .map((features, i) => {
                const output = features.get(config.configuredOutputId) || [];
                const nSamples = request.audioFormat.length;
                const progress = nSamples != null ?
                    {
...
exports.PiperStreamingService = PiperStreamingService;
...
var Observable_1 = __webpack_require__("rCTf");
var toPromise_1 = __webpack_require__("nFIP");
...
var PiperVampService_1 = __webpack_require__("kMRR");
exports.PiperVampService = PiperVampService_1.PiperVampService;
__export(__webpack_require__("3xc6"));
var FeatureExtractorService_1 = __webpack_require__("+MSI");
exports.FeatureExtractorService = FeatureExtractorService_1.FeatureExtractorService;
var PiperClient_1 = __webpack_require__("hdBY");
exports.PiperClient = PiperClient_1.PiperClient;
var HigherLevelUtilities_1 = __webpack_require__("UNpC");
exports.segment = HigherLevelUtilities_1.segment;
exports.process = HigherLevelUtilities_1.process;
exports.collect = HigherLevelUtilities_1.collect;
__export(__webpack_require__("23ZG"));
var Timestamp_1 = __webpack_require__("yAD5");
exports.fromSeconds = Timestamp_1.fromSeconds;
exports.fromFrames = Timestamp_1.fromFrames;
exports.toSeconds = Timestamp_1.toSeconds;
...
class PiperClient {
    ...
    configure(request) {
        return this.service.configure(request)
            ...
            for (let output of response.outputList) {
                this.timeAdjusters.set(output.basic.identifier, FeatureTimeAdjuster_1.createFeatureTimeAdjuster(output.configured, request.configuration.framing.stepSize / this.handleToSampleRate.get(request.handle)));
            }
            return response;
        });
    }
    process(request) {
        ...
        return this.service.process(request).then(response => {
            this.adjustFeatureTimes(response.features, response.handle, request.processInput.timestamp);
            return {
                handle: request.handle,
    ...
    finish(request) {
        return this.service.finish(request).then(response => {
            this.adjustFeatureTimes(response.features, response.handle);
    ...
    adjustFeatureTimes(features, handle, inputTimestamp) {
        for (let [i, featureList] of features.entries()) {
            const adjuster = this.timeAdjusters.get(i);
            featureList.map((feature) => {
                ...
                if (this.isFrequencyDomainExtractor(handle)) {
                    ...
                    feature.timestamp = Timestamp_1.fromSeconds(Timestamp_1.toSeconds(feature.timestamp) + offset);
                }
            });
        }
    }
    static isInputDomainAdapted(adapterFlags) {
        return adapterFlags.length > 0
            && (adapterFlags.includes(FeatureExtractor_1.AdapterFlags.AdaptAll)
                || adapterFlags.includes(FeatureExtractor_1.AdapterFlags.AdaptAllSafe)
                || adapterFlags.includes(FeatureExtractor_1.AdapterFlags.AdaptInputDomain));
    }
    convertProcessInputToFrequencyDomain(processInput) {
    ...
    isFrequencyDomainExtractor(handle) {
        return this.handleToStaticData
            .get(handle)
            .inputDomain === FeatureExtractor_1.InputDomain.FrequencyDomain;
    }
}
exports.PiperClient = PiperClient;
//# sourceMappingURL=PiperClient.js.map
...
Object.defineProperty(exports, "__esModule", { value: true });
const JsonProtocol_1 = __webpack_require__("3xc6");
const FeatureExtractor_1 = __webpack_require__("23ZG");
const FeatureExtractorService_1 = __webpack_require__("+MSI");
var Allocator;
(function (Allocator) {
    Allocator[Allocator["ALLOC_NORMAL"] = 0] = "ALLOC_NORMAL";
    Allocator[Allocator["ALLOC_STACK"] = 1] = "ALLOC_STACK";
    Allocator[Allocator["ALLOC_STATIC"] = 2] = "ALLOC_STATIC";
    Allocator[Allocator["ALLOC_DYNAMIC"] = 3] = "ALLOC_DYNAMIC";
    Allocator[Allocator["ALLOC_NONE"] = 4] = "ALLOC_NONE";
})(Allocator = exports.Allocator || (exports.Allocator = {}));
class PiperVampFeatureExtractor {
    constructor(module, sampleRate, pluginKey) {
        this.module = module;
        pluginKey = pluginKey ? pluginKey : list(module, {}).available[0].key;
        const response = JsonProtocol_1.Deserialise.LoadResponse(jsonRequest(module, JsonProtocol_1.Serialise.LoadRequest({
            key: pluginKey,
    ...
    configure(configuration) {
        const response = JsonProtocol_1.Deserialise.ConfigurationResponse(jsonRequest(this.module, JsonProtocol_1.Serialise.ConfigurationRequest({
            handle: this.handle,
            configuration: configuration
        })));
        return {
            outputs: new Map(response.outputList
                .map(output => [output.basic.identifier, output.configured])),
            framing: response.framing
        };
    }
    getDefaultConfiguration() {
    ...
    process(block) {
        return JsonProtocol_1.Deserialise.ProcessResponse(rawProcess(this.module, {
            handle: this.handle,
            processInput: block
        })).features;
    }
    finish() {
        const response = JsonProtocol_1.Deserialise.FinishResponse(jsonRequest(this.module, JsonProtocol_1.Serialise.FinishRequest({ handle: this.handle })));
        return response.features;
    }
}
exports.PiperVampFeatureExtractor = PiperVampFeatureExtractor;
class PiperVampSynchronousService {
    constructor(module) {
        this.module = module;
    }
    list(request) {
        return JsonProtocol_1.Deserialise.ListResponse(jsonRequest(this.module, JsonProtocol_1.Serialise.ListRequest(request)));
    }
    load(request) {
        return JsonProtocol_1.Deserialise.LoadResponse(jsonRequest(this.module, JsonProtocol_1.Serialise.LoadRequest(request)));
    }
    configure(request) {
        return JsonProtocol_1.Deserialise.ConfigurationResponse(jsonRequest(this.module, JsonProtocol_1.Serialise.ConfigurationRequest(request)));
    }
    process(request) {
        return JsonProtocol_1.Deserialise.ProcessResponse(rawProcess(this.module, request));
    }
    finish(request) {
        return JsonProtocol_1.Deserialise.FinishResponse(jsonRequest(this.module, JsonProtocol_1.Serialise.FinishRequest(request)));
    }
}
exports.PiperVampSynchronousService = PiperVampSynchronousService;
class PiperVampService extends FeatureExtractorService_1.FakeAsyncService {
    constructor(module) {
        super(new PiperVampSynchronousService(module));
    }
}
exports.PiperVampService = PiperVampService;
function list(module, request) {
    return JsonProtocol_1.Deserialise.ListResponse(jsonRequest(module, JsonProtocol_1.Serialise.ListRequest(request)));
}
exports.list = list;
const freeJson = (emscripten, ptr) => emscripten.ccall("piperFreeJson", "void", ["number"], [ptr]);
function jsonRequest(emscripten, request) {
    const doRequest = emscripten.cwrap("piperRequestJson", "number", ["number"]);
    const requestJson = emscripten.allocate(emscripten.intArrayFromString(request), "i8", Allocator.ALLOC_NORMAL);
    const responseJson = doRequest(requestJson);
    emscripten._free(requestJson);
    const jsonString = emscripten.Pointer_stringify(responseJson);
    freeJson(emscripten, responseJson);
    return jsonString;
}
function rawProcess(emscripten, request) {
    const doProcess = emscripten.cwrap("piperProcessRaw", "number", ["number", "number", "number", "number"]);
    const nChannels = request.processInput.inputBuffers.length;
    const nFrames = request.processInput.inputBuffers[0].length;
    const buffersPtr = emscripten._malloc(nChannels * 4);
    const buffers = new Uint32Array(emscripten.HEAPU8.buffer, buffersPtr, nChannels);
    for (let i = 0; i < nChannels; ++i) {
        const framesPtr = emscripten._malloc(nFrames * 4);
        const frames = new Float32Array(emscripten.HEAPU8.buffer, framesPtr, nFrames);
        frames.set(request.processInput.inputBuffers[i]);
        buffers[i] = framesPtr;
    }
    const responseJson = doProcess(request.handle, buffersPtr, request.processInput.timestamp.s, request.processInput.timestamp.n);
    for (let i = 0; i < nChannels; ++i) {
...
//# sourceMappingURL=PiperVampService.js.map
...
const typedArrayReducer = (acc, val) => {
    return Float32Array.of(...acc, ...val);
};
/* unused harmony export typedArrayReducer */

const inPlaceTypedArrayReducer = (acc, val, i) => {
    acc.set(val, i);
    return acc;
};
...
Object.defineProperty(exports, "__esModule", { value: true });
class WebWorkerStreamingServer {
    constructor(workerScope, service) {
        this.service = service;
        this.scope = workerScope;
        const onMessageToWrap = this.scope.onmessage;
...
exports.WebWorkerStreamingServer = WebWorkerStreamingServer;
//# sourceMappingURL=WebWorkerStreamingServer.js.map
...
const cachedHann = memoise(hann);
function applyHannWindowTo(buffer) {
    return multiplyMutating(buffer, cachedHann(buffer.length));
}
exports.applyHannWindowTo = applyHannWindowTo;
function cyclicShiftInPlace(buffer) {
    const midIndex = Math.floor(0.5 + 0.5 * buffer.length);
    const secondHalf = buffer.slice(midIndex);
...
exports.cyclicShiftInPlace = cyclicShiftInPlace;
//# sourceMappingURL=FftUtilities.js.map
...
Object.defineProperty(exports, "__esModule", { value: true });
const Timestamp_1 = __webpack_require__("yAD5");
class ProcessInputBuffersAdjuster {
    constructor(config) {
        this.blockSize = config.framing.blockSize;
        this.stepSize = config.framing.stepSize;
        this.offset = Math.floor(0.5 * this.blockSize);
        this.buffers = [...Array(config.channelCount)].map(() => new Float32Array(this.blockSize + this.offset));
    }
    adjust(input) {
        return {
...
exports.ProcessInputBuffersAdjuster = ProcessInputBuffersAdjuster;
class ProcessInputTimestampAdjuster {
    constructor(blockSize, sampleRate) {
        this.adjustmentSeconds = 0.5 * (blockSize / sampleRate);
    }
    adjust(input) {
        return {
            timestamp: Timestamp_1.fromSeconds(Timestamp_1.toSeconds(input.timestamp) + this.adjustmentSeconds),
            inputBuffers: input.inputBuffers
        };
    }
}
exports.ProcessInputTimestampAdjuster = ProcessInputTimestampAdjuster;
//# sourceMappingURL=ProcessInputAdjuster.js.map
 ...
 * audio sample rate.
 */
function fromFrames(frame, frameRate) {
    return fromSeconds(frame / frameRate);
}
exports.fromFrames = fromFrames;
/**
 ...
 */
function toFrames(timestamp, frameRate) {
 ...
 */
function canonicalise(noncanon) {
